...
@section('title', 'Ubah Pengajuan Transaksi')
@section('page-title', 'Ubah Pengajuan')

@section('breadcrumbs')
<ol class="breadcrumb custom-breadcrumb">
    <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
    <li class="breadcrumb-item"><a href="{{ route('submissions.index') }}">Pengajuan Saya</a></li>
    <li class="breadcrumb-item"><a href="{{ route('submissions.show', $submission) }}">#{{ $submission->submission_number }}</a></li>
    ...
    <div class="col-12 col-xl-9">
        <div class="card border-0 shadow-sm" style="border-radius: 12px;">
            <div class="card-header bg-white border-0 py-3 px-4">
                <h5 class="fw-bold text-dark mb-0"><i class="bi bi-pencil-square text-warning me-2"></i>Form Ubah Pengajuan</h5>
            </div>
            
            <div class="card-body p-4">
                <form id="submissionForm" method="POST" action="{{ route('submissions.update', $submission) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="hidden" name="requested_amount" id="requested_amount" value="{{ old('requested_amount', (int)$submission->requested_amount) }}">
                        </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="description" id="description" class="form-control bg-light @error('description') is-invalid @enderror" rows="5" placeholder="Tuliskan detail keperluan pengeluaran..." required>{{ old('description', $submission->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    @if($submission->submissionFiles->isNotEmpty())
                        <div class="mb-3">
                            <label class="form-label fw-semibold text-muted small">Berkas Saat Ini</label>
                            <div class="d-flex flex-wrap gap-2">
                                @foreach($submission->submissionFiles as $file)
                                    <div class="border d-flex align-items-center gap-2 p-2 rounded bg-light" style="max-width: 300px;">
                                        <i class="bi bi-file-earmark-check text-success fs-5"></i>
                                        <div class="text-truncate" style="font-size: 0.8rem;">
                                            <a href="{{ route('submissions.download', $file) }}" class="fw-semibold text-dark text-decoration-none" title="Download File">{{ $file->file_name }}</a>
                    ...
                    <div class="mb-4">
                        <label for="attachments" class="form-label fw-semibold text-muted small">Unggah Berkas Baru <span class="text-muted">(Akan mengganti berkas saat ini)</span></label>
                    ...
                    <div class="d-flex justify-content-end gap-2 border-top pt-4">
                        <a href="{{ route('submissions.show', $submission) }}" class="btn btn-outline-premium">
                            Kembali
                        </a>
