    ...
    <div class="card-body p-4">
        ...
        <form method="GET" action="{{ route('approvals.index') }}" class="row g-3">
            ...
            <div class="col-12 col-md-2 d-flex align-items-end gap-2">
                ...
                <a href="{{ route('approvals.index') }}" class="btn btn-outline-premium w-100 py-2 d-flex align-items-center justify-content-center" title="Reset Filter">
...
<div class="card custom-table-card border-0">
    <div class="card-header bg-white border-0 py-3 px-4">
        <h5 class="fw-bold text-dark mb-0">Antrian Persetujuan (Role: {{ Auth::user()->roles->first()->name ?? 'N/A' }})</h5>
    </div>
            ...
            <tbody>
                @forelse($submissions as $submission)
                    <tr>
                        ...
                        <td class="fw-bold text-primary">Rp {{ number_format($submission->requested_amount, 0, ',', '.') }}</td>
                        <td>{{ $submission->submission_date->format('d F Y') }}</td>
                        <td class="text-end">
                            <a href="{{ route('approvals.show', $submission) }}" class="btn btn-sm btn-primary-premium py-1 px-3 border-0">
