...
Core Features
Granular RBAC: 5 default roles (Staff, Supervisor, Manager, Director, Finance) mapped to specific granular permissions.
Dynamic Workflow Engine: Expense requests route automatically based on Category and Amount thresholds:
...
Pessimistic Locking Budget Checks: Budget limits are locked and validated in a DB transaction at payment validation time. If category budget is insufficient, the system transitions the status to Rejected (Business Rule 5 & 8).
Secure Supporting Documents Upload: Prevents executable script uploads (sh, php, exe, cgi, bat, py, js) and stores files securely inside private storage (preventing direct web URL leaks).
Interactive UI: Responsive navbar/sidebar, SweetAlert2 confirmation dialogs, toast notifications, persistent Dark Mode setting, and dynamic Chart.js reporting.
Programmatic REST API: Fully guarded by Sanctum tokens for remote submissions and budget monitoring.
...
categories: Expense categories (e.g. PO Produk, Operasional Kantor, etc.).
budgets: Allocated limits per category per fiscal year. Unique composite index on (category_id, fiscal_year).
submissions: Holds request records, amounts, description, and status. Refers to users and categories (onDelete: restrict to prevent losing historical request data).
submission_files: Upload paths, file types, and size. Refers to submissions (onDelete: cascade).
approvals: History trails of approvals. Refers to submissions (onDelete: cascade) and users (onDelete: restrict).
payments: Settlement reference bank numbers and dates. Refers to submissions (onDelete: cascade) and users (onDelete: restrict).
activity_logs: Records actions, IP addresses, and User Agents for audit logs.
